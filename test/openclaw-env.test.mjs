...
test('help keeps the happy path above advanced env tuning noise', () => {
  ...
  assert.match(run.stdout, /Get started:\s+idlewatch quickstart --no-tui(?:\s|$)/)
...
test('help preserves one-off command hints under npm exec', () => {
  ...
  assert.equal(run.status, 0, run.stderr)
  assert.match(run.stdout, /Get started:\s+npx idlewatch quickstart --no-tui(?:\s|$)/)
  assert.doesNotMatch(run.stdout, /# plain text fallback/)
  assert.doesNotMatch(run.stdout, /Get started:\s+npx idlewatch quickstart\s*$/m)
  assert.doesNotMatch(run.stdout, /Get started:\s+idlewatch quickstart\s*$/m)
})
...
test('install-agent help/runtime preserve one-off command hints under npm exec', () => {
  ...
  try {
    ...
    assert.match(help.stdout, /Set up now:\s+npx idlewatch quickstart --no-tui(?:\s|$)/)
...
`, { mode: 0o755 })
        ...
        assert.match(runtime.stderr, /Set up now:\s+npx idlewatch quickstart --no-tui(?:\s|$)/)
        assert.doesNotMatch(runtime.stderr, /# plain text fallback/)
        assert.doesNotMatch(runtime.stderr, /Set up now:\s+npx idlewatch quickstart\s*$/m)
...
test('install-agent help keeps custom saved-config npx handoffs literally runnable', () => {
  ...
  try {
    ...
    assert.equal(help.status, 0, help.stderr)
    assert.match(help.stdout, new RegExp(`Set up now:\\s+${escapeRegex(expectedPrefix)} npx idlewatch quickstart --no-tui(?:\\s|$)`))
    assert.doesNotMatch(help.stdout, /# plain text fallback/)
    assert.doesNotMatch(help.stdout, new RegExp(`Set up now:\\s+${escapeRegex(expectedPrefix)} npx idlewatch quickstart\\s*$`, 'm'))
...
test('install-agent help keeps the durable setup path short and clear', () => {
  ...
  assert.match(run.stdout, /Turns on background mode on macOS using your saved setup\./)
  ...
  assert.match(run.stdout, /If you've already finished setup, this turns on background mode right away\./)
  ...
  assert.match(run.stdout, /Set up now:\s+idlewatch quickstart --no-tui(?:\s|$)/)
  assert.doesNotMatch(run.stdout, /# plain text fallback/)
  ...
  assert.doesNotMatch(run.stdout, /Set up now:\s+idlewatch quickstart\s*$/m)
...
test('install-agent help in npx context points straight to the durable path', () => {
  ...
  assert.match(run.stdout, /Set up now:\s+npx idlewatch quickstart --no-tui(?:\s|$)/)
  assert.doesNotMatch(run.stdout, /# plain text fallback/)
...
test('install-agent refuses disposable npm exec paths and explains the durable path', () => {
  ...
  try {
    ...
    assert.match(run.stderr, /Set up now:\s+npx idlewatch quickstart --no-tui(?:\s|$)/)
...
test('install-agent runtime in npx context keeps custom saved-config handoffs literally runnable', () => {
  ...
  try {
    ...
    assert.notEqual(run.status, 0)
    assert.match(run.stderr, new RegExp(`Set up now:\\s+${escapeRegex(expectedPrefix)} npx idlewatch quickstart --no-tui(?:\\s|$)`))
    assert.doesNotMatch(run.stderr, /# plain text fallback/)
    assert.match(run.stderr, /Install once:\s+npm install -g idlewatch/)
...
test('install-agent follow-up uses source checkout command path', () => {
  ...
  try {
    ...
    assert.doesNotMatch(run.stdout, /# plain text fallback/, 'should keep the install-before-setup handoff to one setup command in a source checkout')
...
test('status command preserves installed-but-waiting-for-setup state after install-agent ran before setup', () => {
  ...
  try {
    ...
    assert.doesNotMatch(status.stdout, /Finish setup:\s+idlewatch quickstart\s*$/m)
...
test('configure --no-tui fails clearly when no saved setup exists yet', () => {
  ...
  try {
    for (const envPatch of [
      ...
      assert.doesNotMatch(run.stderr, /Or use idlewatch quickstart --no-tui if you prefer plain text\./)
      assert.doesNotMatch(run.stderr, /Start with idlewatch quickstart\./)
      assert.doesNotMatch(run.stderr, /# plain text fallback/)
...
test('configure --no-tui keeps the calmer setup-first recovery copy in true npx mode when no saved setup exists yet', () => {
  ...
  try {
    ...
    assert.doesNotMatch(run.stderr, /Or use npx idlewatch quickstart --no-tui if you prefer plain text\./)
    assert.doesNotMatch(run.stderr, /Start with npx idlewatch quickstart\./)
    assert.doesNotMatch(run.stderr, /# plain text fallback/)
