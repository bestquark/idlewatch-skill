...
test('install-agent follow-up uses source checkout command path', () => {
  ...
  try {
    ...
    assert.ok(run.stdout.includes('Then start:   idlewatch install-agent'), 'should keep the follow-up start hint on the calmer product command in a source checkout')
...
test('install-agent does not claim background is running when launchd still reports not loaded', () => {
  ...
  try {
    ...
    assert.ok(install.stdout.includes('Start:        idlewatch install-agent'), 'should keep the start hint on the calmer product command when the agent is installed but not loaded')
...
test('quickstart completion stays honest when a LaunchAgent was installed before setup', () => {
  ...
  try {
    ...
    assert.match(run.stdout, /idlewatch run\s+Run in foreground/)
...
test('configure success says to refresh an already-running background agent', () => {
  ...
  try {
    ...
    assert.equal(quickstart.status, 0, quickstart.stderr)
    assert.match(quickstart.stdout, /Use it now:/)
    assert.match(quickstart.stdout, /idlewatch run\s+Run in foreground/)
    assert.match(quickstart.stdout, /For background mode:/)
    ...
    if (launchAgentWasRunning) {
      ...
      assert.match(configure.stdout, /Use it now:/)
      assert.match(configure.stdout, /idlewatch run\s+Run in foreground/)
      assert.match(configure.stdout, /For background mode:/)
...
`)
        ...
        assert.doesNotMatch(withInstalledAgent.stdout, /Turn on background mode:\s+idlewatch install-agent/)
      } finally {
        rmSync(fakeBinDir, { recursive: true, force: true })
      }
    }
