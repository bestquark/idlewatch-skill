...
test('help keeps the happy path above advanced env tuning noise', () => {
  ...
  assert.doesNotMatch(run.stdout, /Get started:\s+idlewatch quickstart --no-tui/)
...
`, { mode: 0o755 })
        ...
        assert.match(runtime.stdout, /Set up now:\s+npx idlewatch quickstart(?:\s|$)/)
        assert.match(runtime.stdout, /npx idlewatch quickstart --no-tui\s+# plain text fallback/)
...
test('install-agent help keeps the durable setup path short and clear', () => {
  ...
  assert.doesNotMatch(run.stdout, /Set up now:\s+idlewatch quickstart --no-tui/)
...
test('install-agent follow-up uses source checkout command path', () => {
  ...
  try {
    ...
    assert.ok(run.stdout.includes('Finish setup: idlewatch quickstart'), 'should lead with the friendlier setup hint on the calmer product command in a source checkout')
    assert.ok(run.stdout.includes('                 idlewatch quickstart --no-tui   # plain text fallback'), 'should keep the plain-text setup fallback one line below in a source checkout')
...
test('status command preserves installed-but-waiting-for-setup state after install-agent ran before setup', () => {
  ...
  try {
    ...
    assert.match(status.stdout, /idlewatch quickstart --no-tui   # plain text fallback/)
...
test('source checkout does not drift into npx wording from a stray npm_command env alone', () => {
  ...
  try {
    ...
    assert.equal(run.status, 0, run.stderr)
    assert.match(run.stdout, /Get started:\s+idlewatch quickstart(?:\s|$)/)
    assert.match(run.stdout, /idlewatch quickstart --no-tui   # plain text fallback/)
