...
test('install-agent follow-up uses source checkout command path', () => {
  ...
  try {
    ...
    assert.ok(run.stdout.includes(`Save setup:   ${SOURCE_CMD} quickstart --no-tui`), 'should keep the setup hint copy-pasteable for the current source checkout')
    assert.ok(run.stdout.includes(`Run now:      ${SOURCE_CMD} run`), 'should keep the foreground run hint on the current source-checkout command')
    assert.ok(run.stdout.includes(`Then start:   ${SOURCE_CMD} install-agent`), 'should keep the follow-up start hint on the current source-checkout command')
    ...
    assert.doesNotMatch(run.stdout, /Save setup:\s+idlewatch quickstart --no-tui/)
  } finally {
    rmSync(tempDir, { recursive: true, force: true })
  }
})
