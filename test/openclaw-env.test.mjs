...
test('help keeps the happy path above advanced env tuning noise', () => {
  ...
  assert.match(run.stdout, /# plain text fallback/)
...
test('install-agent help/runtime preserve one-off command hints under npm exec', () => {
  ...
  try {
    ...
    assert.match(help.stdout, /# plain text fallback/)
...
`, { mode: 0o755 })
        ...
        assert.match(runtime.stderr, /# plain text fallback/)
...
test('install-agent help keeps custom saved-config npx handoffs literally runnable', () => {
  ...
  try {
    ...
    assert.equal(help.status, 0, help.stderr)
    assert.match(help.stdout, new RegExp(`Set up now:\\s+${escapeRegex(expectedPrefix)} npx idlewatch quickstart\\n\\s+${escapeRegex(expectedPrefix)} npx idlewatch quickstart --no-tui\\s+# plain text fallback`))
    assert.match(help.stdout, /# plain text fallback/)
    assert.doesNotMatch(help.stdout, new RegExp(`Set up now:\\s+${escapeRegex(expectedPrefix)} npx idlewatch quickstart --no-tui(?:\\s|$)`))
    assert.match(help.stdout, new RegExp(`Then turn on background mode:\\s+${escapeRegex(expectedPrefix)} idlewatch install-agent`))
...
test('install-agent help keeps the durable setup path short and clear', () => {
  ...
  assert.match(run.stdout, /Turns on background mode on macOS with your saved setup\./)
  assert.doesNotMatch(run.stdout, /Turns on background mode on macOS using your saved setup\./)
  assert.doesNotMatch(run.stdout, /Turns on background mode on macOS\./)
  ...
  assert.match(run.stdout, /If you've already finished setup, this works right away\./)
...
test('uninstall-agent help in npx context keeps the durable off-ramp literally runnable (help surface)', () => {
  ...
  assert.match(run.stdout, /Turn background mode back on later: install once with npm install -g idlewatch, then run idlewatch install-agent\./)
  assert.doesNotMatch(run.stdout, /Install once:\s+npm install -g idlewatch/)
...
test('quickstart help stays clean in non-TTY mode', () => {
  ...
  assert.match(run.stdout, /Uses the simple setup flow\. For unattended runs, set IDLEWATCH_ENROLL_\* env vars first\./)
...
test('reconfigure help stays clean in non-TTY mode', () => {
  ...
  assert.equal(run.status, 0, run.stderr)
  assert.match(run.stdout, /reconfigure — Update setup \(alias for configure\)\n\nUsage:\s+idlewatch reconfigure\n\s+idlewatch reconfigure --no-tui\s+# plain text fallback\n/)
  ...
  assert.doesNotMatch(run.stdout, /Usage:\s+idlewatch reconfigure --no-tui(?:\s|$)/)
  assert.match(run.stdout, /Updates device name, metrics, and your optional cloud link\./)
  assert.doesNotMatch(run.stdout, /Re-opens setup to change device name, metrics, and your optional cloud link\./)
  assert.match(run.stdout, /Uses the simple setup flow\. For unattended runs, set IDLEWATCH_ENROLL_\* env vars first\./)
  assert.doesNotMatch(run.stdout, /Uses simple prompts\. Set IDLEWATCH_ENROLL_\* env vars first\./)
...
test('install-agent refuses disposable npm exec paths and explains the durable path', () => {
  ...
  try {
    ...
    assert.match(run.stderr, /# plain text fallback/)
    ...
    assert.match(run.stderr, /Then turn on background mode:\s+idlewatch install-agent/)
...
test('install-agent runtime in npx context keeps custom saved-config handoffs literally runnable', () => {
  ...
  try {
    ...
    assert.notEqual(run.status, 0)
    assert.match(run.stderr, new RegExp(`Set up now:\\s+${escapeRegex(expectedPrefix)} npx idlewatch quickstart\\n\\s+${escapeRegex(expectedPrefix)} npx idlewatch quickstart --no-tui\\s+# plain text fallback`))
    assert.match(run.stderr, /# plain text fallback/)
    assert.match(run.stderr, /Install once:\s+npm install -g idlewatch/)
    assert.match(run.stderr, new RegExp(`Then turn on background mode:\\s+${escapeRegex(expectedPrefix)} idlewatch install-agent`))
...
test('install-agent follow-up uses source checkout command path', () => {
  ...
  try {
    ...
    assert.ok(run.stdout.includes('Finish setup: idlewatch quickstart'), 'should lead with the calmer setup hint in a source checkout')
    assert.match(run.stdout, /# plain text fallback/, 'should keep the install-before-setup handoff on the plain-text fallback in a source checkout')
    ...
    assert.ok(run.stdout.includes('Check:        idlewatch status   See setup state, background mode state, and last publish result'), 'should show the calmer product status command with the setup-state hint in a source checkout before setup is saved')
...
test('status command preserves installed-but-waiting-for-setup state after install-agent ran before setup', () => {
  ...
  try {
    ...
    assert.match(status.stdout, /# plain text fallback/)
...
test('quickstart keeps the installed-but-not-running wording clear after install-agent ran before setup', () => {
  ...
  try {
    ...
    assert.match(quickstart.stdout, /Turn on background mode:\s+idlewatch install-agent/)
...
test('quickstart uses apply-saved-config wording when install-agent was already loaded before setup finished', () => {
  if (process.platform !== 'darwin') {
    return
  }

  const tempHome = mkdtempSync(path.join(os.tmpdir(), 'idlewatch-quickstart-installed-running-'))
  const fakeBinDir = mkdtempSync(path.join(os.tmpdir(), 'idlewatch-quickstart-running-launchctl-bin-'))
  const fakeLaunchctl = path.join(fakeBinDir, 'launchctl')
  try {
    writeFileSync(fakeLaunchctl, '#!/usr/bin/env bash\nset -euo pipefail\ncmd="${1:-}"\nif [[ "$cmd" == "print" ]]; then\n  cat <<\'EOF\'\npid = 4242\nEOF\n  exit 0\nfi\nif [[ "$cmd" == "bootstrap" || "$cmd" == "enable" || "$cmd" == "bootout" || "$cmd" == "disable" || "$cmd" == "kickstart" ]]; then\n  exit 0\nfi\nexit 0\n', { encoding: 'utf8' })
    chmodSync(fakeLaunchctl, 0o755)
    ...
    const quickstart = spawnSync(process.execPath, [BIN, 'quickstart', '--no-tui'], {
      env: {
        ...
        IDLEWATCH_ENROLL_NON_INTERACTIVE: '1',
        IDLEWATCH_ENROLL_MODE: 'local',
        IDLEWATCH_ENROLL_DEVICE_NAME: 'QA Running Box',
        IDLEWATCH_ENROLL_MONITOR_TARGETS: 'cpu,memory'
      },
      encoding: 'utf8',
      timeout: 20000
    ...
    assert.equal(quickstart.status, 0, quickstart.stderr)
    assert.match(quickstart.stdout, /Background mode:\s+already running/)
    assert.match(quickstart.stdout, /Apply saved config:\s+re-run idlewatch install-agent to apply the saved config/)
    assert.doesNotMatch(quickstart.stdout, /For background mode:\n\s+idlewatch install-agent\s+Turn on background mode/)
    assert.doesNotMatch(quickstart.stdout, /Turn on background mode:\s+idlewatch install-agent/)
  } finally {
    rmSync(fakeBinDir, { recursive: true, force: true })
    rmSync(tempHome, { recursive: true, force: true })
  }
...
test('quickstart names the valid enrollment modes when non-interactive mode is invalid', () => {
  ...
  try {
    ...
    assert.match(run.stderr, /Set IDLEWATCH_ENROLL_MODE=cloud or IDLEWATCH_ENROLL_MODE=local\./)
    assert.equal(fs.existsSync(path.join(tempHome, '.idlewatch', 'idlewatch.env')), false)
  } finally {
    rmSync(tempHome, { recursive: true, force: true })
  }
...
test('quickstart and configure keep one-off runs honest about background install under npm exec env', () => {
  ...
  try {
    ...
    assert.match(quickstart.stdout, /Install once:\s+npm install -g idlewatch/)
    assert.match(quickstart.stdout, /Then turn on background mode: idlewatch install-agent/)
    ...
    assert.match(configure.stdout, /Install once:\s+npm install -g idlewatch/)
    assert.match(configure.stdout, /Then turn on background mode: idlewatch install-agent/)
...
`)
        ...
        assert.match(withInstalledAgent.stdout, /Turn on background mode:\s+idlewatch install-agent/)
...
`, { mode: 0o755 })
    ...
    assert.ok(run.stdout.includes('Turn on background mode:  idlewatch install-agent'), 'should keep the installed-not-running hint explicit about turning on background mode in source checkouts')
