...
test('help keeps the happy path above advanced env tuning noise', () => {
  ...
  assert.match(run.stdout, /install-agent\s+Turn on background mode \(macOS\)/)
...
test('install-agent help keeps the durable setup path short and clear', () => {
  ...
  assert.match(run.stdout, /If setup isn't saved yet, finish setup with idlewatch quickstart --no-tui, then run idlewatch install-agent\./)
...
test('install-agent help in npx context points straight to the durable path', () => {
  ...
  assert.match(run.stdout, /If setup isn't saved yet:\s+idlewatch quickstart --no-tui/)
...
test('quickstart help stays clean in non-TTY mode', () => {
  ...
  assert.match(run.stdout, /Runs non-interactively\. Set IDLEWATCH_ENROLL_\* env vars first\./)
...
test('install-agent follow-up uses source checkout command path', () => {
  ...
  try {
    ...
    assert.ok(run.stdout.includes('Save setup:   idlewatch quickstart --no-tui'), 'should keep the setup hint on the calmer product command in a source checkout')
...
test('quickstart keeps the installed-but-not-running wording clear after install-agent ran before setup', () => {
  ...
  try {
    ...
    assert.match(quickstart.stdout, /It stays off until then\./)
...
test('install-agent does not claim background is running when launchd still reports not loaded', () => {
  ...
  try {
    ...
    assert.ok(install.stdout.includes('Turn on background mode:  idlewatch install-agent'), 'should keep the background-mode hint on the calmer product command when the agent is installed but not loaded')
...
`)
        ...
        assert.match(npxConfigureWithRunningAgent.stdout, /Run now:\s+.*run\s+Run in the foreground/)
...
test('configure success says to refresh an already-running background agent', () => {
  ...
  try {
    ...
    if (launchAgentWasRunning) {
      ...
      assert.match(configure.stdout, /Run now:\s+.*run\s+Run in the foreground/)
...
`, { mode: 0o755 })
    ...
    assert.ok(run.stdout.includes('Turn on background mode:  idlewatch install-agent'), 'should keep the background-mode hint on the calmer product command for the saved background agent in source checkouts')
