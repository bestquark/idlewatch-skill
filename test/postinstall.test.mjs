...
test('postinstall stays CLI-first by default', () => {
  ...
  assert.match(run.stdout, /Set up this device:\n\s+idlewatch quickstart --no-tui(?:\s|$)/)
...
test('postinstall keeps the durable install handoff neat and macOS guidance user-facing', () => {
  ...
  assert.match(run.stdout, /Set up this device:\n\s+idlewatch quickstart --no-tui(?:\s|$)/)
  assert.doesNotMatch(run.stdout, /Set up this device:\n\s+idlewatch quickstart\n/)
  assert.match(run.stdout, /Optional on macOS:\n\s+idlewatch install-agent\s+# start background mode after setup/)
  assert.doesNotMatch(run.stdout, /turn on background mode after setup/)
