...
- ✅ **QA sweep executed (monitor + distribution):** full validation set run for this 20-minute heartbeat.
- ✅ **No source code changes in this cycle.**
- ✅ **Packaging validators remained deterministic:** both packaged app and DMG-install dry-runs validated cleanly with extended retry/timeout profile (`IDLEWATCH_DRY_RUN_TIMEOUT_MS=30000`, 3 attempts, +10s bonus).
...
- ✅ **Feature:** Host + packaged telemetry remains schema-complete and includes OpenClaw provenance (`usageSource`, probe timing/counters, and alert fields) in both `auto` and `off` modes.
- ✅ **Feature:** `usageRefresh` instrumentation is deterministic in this cycle (`usageRefreshAttempted: true`, `usageRefreshRecovered: false`) with `usageNearStale`/`usagePastStaleThreshold` correctly reflected.
- 🐛 **Open:** `openclawUsageAgeMs` remains in sustained stale state after ~258–263s in both host and packaged auto modes; alert remains `warning` (`activity-past-threshold`) while ingestion still reports `ok`.
- 🐛 **Open:** No authenticated Firebase path available on this host (`Firebase is not configured`), so remote write-mode behavior is still unverified here.

### Telemetry validation checks (latest samples)

- Host `node bin/idlewatch-agent.js --dry-run --json`:
  - `cpuPct: 14.98`, `memUsedPct: 56.21`, `memPressurePct: 20`, `openclawUsageAgeMs: 258,439`
  - `usageFreshnessState: stale`, `usageIntegrationStatus: stale`, `usageIngestionStatus: ok`, `usageAlertLevel: warning`, `usageAlertReason: activity-past-threshold`

- Host `IDLEWATCH_OPENCLAW_USAGE=off node bin/idlewatch-agent.js --dry-run --json`:
  - `cpuPct: 4.35`, `memUsedPct: 56.21`, `memPressurePct: 20`, `openclawUsageAgeMs: null`
  - `usageFreshnessState: disabled`, `usageIntegrationStatus: disabled`, `usageIngestionStatus: disabled`, `usageAlertLevel: off`, `usageAlertReason: usage-disabled`

- Packaged `./dist/IdleWatch.app/Contents/MacOS/IdleWatch --dry-run --once --json`:
  - `cpuPct: 17.99`, `memUsedPct: 57.86`, `memPressurePct: 20`, `openclawUsageAgeMs: 263,333`
  - `usageFreshnessState: stale`, `usageIntegrationStatus: stale`, `usageIngestionStatus: ok`, `usageAlertLevel: warning`, `usageAlertReason: activity-past-threshold`

- Packaged with usage off `IDLEWATCH_OPENCLAW_USAGE=off ./dist/IdleWatch.app/Contents/MacOS/IdleWatch --dry-run --once --json`:
  - `cpuPct: 13.04`, `memUsedPct: 57.86`, `memPressurePct: 20`, `openclawUsageAgeMs: null`
  - `usageFreshnessState: disabled`, `usageIntegrationStatus: disabled`, `usageIngestionStatus: disabled`, `usageAlertLevel: off`, `usageAlertReason: usage-disabled`

### DMG packaging risks

1. **High:** Distribution remains unsigned/unnotarized (`MACOS_CODESIGN_IDENTITY`, `MACOS_NOTARY_PROFILE` unset); trusted-install path (`IDLEWATCH_REQUIRE_TRUSTED_DISTRIBUTION=1`) still unproven.
2. **Medium:** Current validator evidence confirms dry-run and checksum health, but no end-to-end Gatekeeper acceptance verification exists without signing/notary credentials.
3. **Low:** Validation depends on local timeout/retry tuning and machine output behavior; keep monitoring for any timing regressions on cleaner CI runners.

### OpenClaw integration gaps

1. **Gap:** No authenticated Firebase/service-account/emulator-backed write-path validation in this host context.
2. **Gap:** Stale/warning state persists during prolonged idle windows and does not recover to fresh within measured sample windows.
3. **Gap:** DMG + OpenClaw trust-chain behavior (signed/notarized artifact + remote ingest verification) remains untested.

### Follow-up / status

1. Continue 20-minute cadence and retain deterministic host/packaged sample pairs each cycle.
2. Resolve Firebase auth path (`FIREBASE_*` or emulator host) and run one pass with OpenClaw write-enabled integration.
3. Execute a signing/notary-enabled packaging run to validate trust-gated distribution behavior in `dmg-install`.
...
## QA cycle update — 2026-02-17 21:50 America/Toronto

### Completed this cycle

- ✅ **QA sweep executed (monitor + distribution):** full validation set run for this heartbeat.
- ✅ **No source code changes in this cycle.**
- ✅ **Packaging deterministic now for both app-launch paths:** `packaged-bundled-runtime` and `validate:dmg-install` completed cleanly on first pass with the timeout/retry profile.
...
- ✅ **Feature:** Host + packaged telemetry shape remains valid with OpenClaw provenance fields in `usage=auto` and `usage=off` modes.
- ✅ **Feature:** Packaged runtime and installed-DMG launch validations are now deterministic on this host profile with no forced fallback path in this cycle.
- ✅ **Feature:** OpenClaw fallback/reprobe flow remains deterministic (`refreshAttempted: true`, `refreshRecovered: false`) with valid stale state transitions.
- 🐛 **Open:** `openclawUsageAgeMs` remains in stale/`warning` state at ~251k–256k ms in host and packaged auto-mode samples.
- 🐛 **Open:** No Firebase/service-account or emulator validation available on this host (`Firebase is not configured`), so remote write-path remains unverified.

### Telemetry validation checks (latest samples)

- Host `node bin/idlewatch-agent.js --dry-run --json`:
  - `cpuPct: 16.86`, `memUsedPct: 53.64`, `memPressurePct: 18`, `openclawUsageAgeMs: 251,157`
  - `usageFreshnessState: stale`, `usageIntegrationStatus: stale`, `usageIngestionStatus: ok`, `usageAlertLevel: warning`, `usageAlertReason: activity-past-threshold`

- Host `IDLEWATCH_OPENCLAW_USAGE=off node bin/idlewatch-agent.js --dry-run --json`:
  - `cpuPct: 25`, `memUsedPct: 53.70`, `memPressurePct: 18`, `openclawUsageAgeMs: null`
  - `usageFreshnessState: disabled`, `usageIntegrationStatus: disabled`, `usageIngestionStatus: disabled`, `usageAlertLevel: off`, `usageAlertReason: usage-disabled`

- Packaged `./dist/IdleWatch.app/Contents/MacOS/IdleWatch --dry-run --once --json`:
  - `cpuPct: 18.38`, `memUsedPct: 54.57`, `memPressurePct: 18`, `openclawUsageAgeMs: 256,040`
  - `usageFreshnessState: stale`, `usageIntegrationStatus: stale`, `usageIngestionStatus: ok`, `usageAlertLevel: warning`, `usageAlertReason: activity-past-threshold`

- Packaged with usage off `IDLEWATCH_OPENCLAW_USAGE=off ./dist/IdleWatch.app/Contents/MacOS/IdleWatch --dry-run --once --json`:
  - `cpuPct: 12.50`, `memUsedPct: 54.54`, `memPressurePct: 18`, `openclawUsageAgeMs: null`
  - `usageFreshnessState: disabled`, `usageIntegrationStatus: disabled`, `usageIngestionStatus: disabled`, `usageAlertLevel: off`, `usageAlertReason: usage-disabled`

### DMG packaging risks

1. **High:** Distribution artifacts continue to be unsigned/unnotarized (`MACOS_CODESIGN_IDENTITY`, `MACOS_NOTARY_PROFILE` unset); Gatekeeper trust path remains unproven.
2. **Medium:** `IDLEWATCH_REQUIRE_TRUSTED_DISTRIBUTION=1` remains untested end-to-end until signing/notary is configured.
3. **Low:** Current validation remains environment/path dependent (restricted-path dry-run behavior in scripts is sensitive to host toolchain and I/O timing under heavier load).

### OpenClaw integration gaps

1. **Gap:** No authenticated Firebase writes or service-account-backed validation in this host environment.
2. **Gap:** Stale/`warning` transition remains long-lived under sustained idle in this host profile.
3. **Gap:** Signed/distribution trust verification and remote OpenClaw write-path remain pending.

### Follow-up / status

1. Keep 20-minute cadence and keep capturing host/packaged stale-to-warning trajectory for policy confirmation.
2. Run one cycle against Firebase emulator or service-account credentials for true end-to-end write-path verification.
3. Execute signing/notary-enabled run to validate `IDLEWATCH_REQUIRE_TRUSTED_DISTRIBUTION=1` behavior.

## QA cycle update — 2026-02-17 21:40 America/Toronto

### Completed this cycle

- ✅ **QA sweep executed (monitor + distribution):** full validation set run for this heartbeat.
- ✅ **No source code changes in this cycle.**
- ✅ **Packaging validators now run end-to-end:** `packaged-bundled-runtime`, `dmg-install`, and `packaged-usage-age-slo` all passed cleanly with current timeout/retry profile.
...
- ✅ **Feature:** Host + packaged telemetry shape remains valid with OpenClaw provenance fields in auto/off modes.
- ✅ **Feature:** Packaged runtime and DMG install checks passed deterministically this cycle (no launch/no-output fallback path).
- ✅ **Feature:** OpenClaw probe fallback behavior remains stable in long-idle periods (reprobes attempted and telemetry remains ingesting).
- 🐛 **Open:** `openclawUsageAgeMs` remains stale in both host and packaged auto modes (`~246k–251k ms`) producing `warning` state.
- 🐛 **Open:** Local credentials remain unavailable; signed/notarized execution path still unverified.

### Telemetry validation checks (latest samples)

- Host `node bin/idlewatch-agent.js --dry-run --json`:
  - `cpuPct: 20.68`, `memUsedPct: 53.82`, `memPressurePct: 19`, `openclawUsageAgeMs: 246,484`
  - `usageFreshnessState: stale`, `usageIntegrationStatus: stale`, `usageIngestionStatus: ok`, `usageAlertLevel: warning`, `usageAlertReason: activity-past-threshold`

- Host `IDLEWATCH_OPENCLAW_USAGE=off node bin/idlewatch-agent.js --dry-run --json`:
  - `cpuPct: 16.67`, `memUsedPct: 53.79`, `memPressurePct: 19`, `openclawUsageAgeMs: null`
  - `usageFreshnessState: disabled`, `usageIntegrationStatus: disabled`, `usageIngestionStatus: disabled`, `usageAlertLevel: off`, `usageAlertReason: usage-disabled`

- Packaged `./dist/IdleWatch.app/Contents/MacOS/IdleWatch --dry-run --once --json`:
  - `cpuPct: 17.87`, `memUsedPct: 53.77`, `memPressurePct: 18`, `openclawUsageAgeMs: 251,360`
  - `usageFreshnessState: stale`, `usageIntegrationStatus: stale`, `usageIngestionStatus: ok`, `usageAlertLevel: warning`, `usageAlertReason: activity-past-threshold`

- Packaged with usage off `IDLEWATCH_OPENCLAW_USAGE=off ./dist/IdleWatch.app/Contents/MacOS/IdleWatch --dry-run --once --json`:
  - `cpuPct: 16.00`, `memUsedPct: 53.98`, `memPressurePct: 18`, `openclawUsageAgeMs: null`
  - `usageFreshnessState: disabled`, `usageIntegrationStatus: disabled`, `usageIngestionStatus: disabled`, `usageAlertLevel: off`, `usageAlertReason: usage-disabled`

### DMG packaging risks

1. **High:** Artifacts remain unsigned/unnotarized (`MACOS_CODESIGN_IDENTITY`, `MACOS_NOTARY_PROFILE` unset), so Gatekeeper trust path remains unproven.
2. **Medium:** `IDLEWATCH_REQUIRE_TRUSTED_DISTRIBUTION=1` remains untested for end-to-end behavior until signing/notary is enabled.
3. **Medium:** Current cycle still depends on local path/toolchain assumptions (node runtime location + temp mount behavior).

### OpenClaw integration gaps

1. **Gap:** No authenticated Firebase or emulator credentials in this host; validation still local-only (`Firebase is not configured`).
2. **Gap:** Long-horizon stale state remains unresolved (`warning` with age near 4.1–4.2 minutes).
3. **Gap:** No remote-verified signed artifact -> OpenClaw ingestion chain proof yet.

### Follow-up / status

1. Keep 20-minute cadence and keep capturing host/packaged stale-to-warning trajectory for threshold behavior confidence.
2. Run a credentials-backed/write-path cycle with service account or Firestore emulator as soon as credentials are available.
3. Execute trust-aware packaging validation with `MACOS_CODESIGN_IDENTITY` + `MACOS_NOTARY_PROFILE` + `IDLEWATCH_REQUIRE_TRUSTED_DISTRIBUTION=1`.

## QA cycle update — 2026-02-17 21:30 America/Toronto

### Completed this cycle

- ✅ **QA sweep executed (monitor + distribution):** full validation set run for this 20-minute heartbeat.
- ✅ **No source code changes in this cycle.**
- ✅ **Packaged and DMG validators completed deterministically** (no `dry-run` output-timeout fallback this pass).
...
- ✅ **Feature:** Host and packaged launchers are healthy and pass all validation suites in this run, with dry-run telemetry available in both auto/off modes.
- ✅ **Feature:** `openclawUsageAgeMs` is currently fresh in both host and packaged auto-mode samples (`~35–37s`) and fresh/ok alert state is stable.
- ✅ **Feature:** OpenClaw offline/local mode remains explicit (`Firebase is not configured`) and does not block runtime validation.
- 🐛 **Open:** No authenticated Firestore/cloud write validation can be executed on this host in this pass.
- 🐛 **Open:** DMG trust path remains unverified because signing/notarization env vars are still unset.
...
## QA cycle update — 2026-02-17 21:20 America/Toronto

### Completed this cycle

- ✅ **QA sweep executed (monitor + distribution):** full validation set run for this 20-minute heartbeat.
- ✅ **No source code changes in this cycle.**
- ✅ **Packaged validators remain deterministic on this host** (no fallback behavior triggered in this pass).
...
- ✅ **Feature:** Host and packaged launchers continue to pass all packaging + recovery/alert suites with stable dry-run output.
- ✅ **Feature:** Offline telemetry mode remains explicit (`Firebase is not configured`) and does not impact runtime validation path.
- 🐛 **Open:** `openclawUsageAgeMs` is now clearly exceeding stale thresholds in both host and packaged modes (around ~250k ms), producing warning-level stale state.
- 🐛 **Open:** No authenticated Firestore/credentials-backed write validation in this environment.
...
## QA cycle update — 2026-02-17 21:10 America/Toronto

### Completed this cycle

- ✅ **QA sweep executed (monitor + distribution):** full validation set run for this heartbeat.
- ✅ **No source changes in this cycle.**
- ✅ `packaged-bundled-runtime` and `dmg-install` now pass on first pass with timeout/retry settings used on this host (`IDLEWATCH_DRY_RUN_TIMEOUT_MS=30000`, defaults for max attempts/backoff).
...
- ✅ **Feature:** OpenClaw telemetry continues to emit expected schema fields on host and packaged launchers.
- ✅ **Feature:** both packaged validators now complete deterministically in this environment on this heartbeat.
- 🐛 **Open:** `openclawUsageAgeMs` still rises into sustained stale windows (`~283k–293k ms`), keeping OpenClaw state in `stale` + `warning` after prolonged inactivity.
- 🐛 **Open:** distribution remains unsigned/unnotarized by default, so Gatekeeper trust path remains unvalidated end-to-end.
...
2. **Medium:** Evidence remains toolchain/environment coupled; a slower or cleaner macOS runner may still surface timing edge cases despite today’s clean pass.
