...
- For local scripted workflows, `npm run validate:openclaw-release-gates:all` and `npm run validate:packaged-openclaw-release-gates:all` provide host+packaged and fresh+reuse coverage in one command.
