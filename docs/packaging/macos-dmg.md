...
- Core validation coverage also runs `npm run validate:openclaw-stats-ingestion` and `npm run validate:packaged-openclaw-stats-ingestion` to guard the `stats --json` fallback path when status/usage endpoints diverge in both host and packaged runtime modes.
