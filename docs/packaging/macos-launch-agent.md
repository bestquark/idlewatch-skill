...
Config changes are picked up on the next start. After running `idlewatch quickstart --no-tui` or changing settings, re-run the install script once to turn background mode on or apply the saved config.
