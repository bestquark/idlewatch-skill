...
- `npm run validate:packaged-openclaw-stats-ingestion` validates packaged-app stats fallback ingestion under a mocked `openclaw` binary (end-to-end packaged dry-run + `stats --json` command selection), including both `status.result` and `status.current` stats payload layouts.
...
- Dry-run gate timeout via `IDLEWATCH_DRY_RUN_TIMEOUT_MS` (default: `15000`)
  ...
  - OpenClaw-specific release helpers (`validate:openclaw-release-gates` and `validate:packaged-openclaw-release-gates`) default to `60000` so probe latency on slower hosts does not create false release-gate failures.
