...
- The trusted pipeline now also runs `npm run validate:packaged-openclaw-stats-ingestion` and `npm run validate:packaged-openclaw-cache-recovery-e2e` before upload to validate stats fallback and stale-cache recovery behavior under signed build conditions (with `IDLEWATCH_SKIP_PACKAGE_MACOS=1` so these checks validate the already-built signed artifact directly).
